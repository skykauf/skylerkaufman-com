...
async function searchPlayers(filters = {}) {
  ...
  if (filters.gender !== undefined && String(filters.gender).trim() !== "") {
    params.push(String(filters.gender).trim());
    where.push(`gender::text = $${params.length}`);
  }
  if (filters.min_height_cm !== undefined && String(filters.min_height_cm).trim() !== "") {
    params.push(toNum(filters.min_height_cm, 0));
    where.push(`height_cm >= $${params.length}`);
  }
  if (filters.max_height_cm !== undefined && String(filters.max_height_cm).trim() !== "") {
    params.push(toNum(filters.max_height_cm, 9999));
    where.push(`height_cm <= $${params.length}`);
  }
  if (filters.min_weight_kg !== undefined && String(filters.min_weight_kg).trim() !== "") {
    params.push(toNum(filters.min_weight_kg, 0));
    where.push(
      `CASE
         WHEN (payload->>'Weight') ~ '^\\d+(\\.\\d+)?$'
         THEN (payload->>'Weight')::numeric / 1000000
         ELSE NULL
       END >= $${params.length}`
    );
  }
  if (filters.max_weight_kg !== undefined && String(filters.max_weight_kg).trim() !== "") {
    params.push(toNum(filters.max_weight_kg, 9999));
    where.push(
      `CASE
         WHEN (payload->>'Weight') ~ '^\\d+(\\.\\d+)?$'
         THEN (payload->>'Weight')::numeric / 1000000
