...
const TOOL_DEFS = [
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          active_within_days: { type: "integer", description: "Optional recency filter for active players." },
  ...
  },
  {
    ...
    function: {
      name: "active_players",
      description:
        "Return active FIVB beach volleyball players who played recently (default last 365 days), optionally filtered by country/gender.",
      parameters: {
        type: "object",
        properties: {
          ...
          days_back: { type: "integer", description: "Recency window in days, default 365." },
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Return inactive FIVB beach volleyball players whose last match was before the cutoff (default 365 days).",
      parameters: {
        type: "object",
        properties: {
          country_code: { type: "string", description: "Optional 3-letter country code (USA, BRA, NOR)." },
          gender: { type: "string", description: "Optional gender filter (men/women or M/W/0/1)." },
          days_back: { type: "integer", description: "Inactivity threshold in days, default 365." },
          limit: { type: "integer", description: "Max rows, default 25." },
        },
      },
    },
...
function parseDaysBack(value, fallback = 365, maxDays = 3650) {
...
async function runTool(name, args, onProgress) {
  ...
  if (name === "top_players_by_country") {
    ...
    const activeDays = parseDaysBack(args.active_within_days, 0);
    if (activeDays > 0) {
      params.push(activeDays);
      sql += ` AND last_match_played_at >= current_date - ($${params.length}::int * interval '1 day')`;
    }
    params.push(limit);
  ...
  if (name === "best_finishes_by_player") {
    ...
    const out = await queryRows(
      ...
      WITH ids AS (
        ...
        LEFT JOIN mart.player_elo_latest pel
          ON pel.player_id = p.player_id
        WHERE p.full_name ILIKE $1
          AND ($3::int <= 0 OR pel.last_match_played_at >= current_date - ($3::int * interval '1 day'))
        ORDER BY full_name
        LIMIT 10
      )
      SELECT
  ...
  if (name === "active_players") {
    ...
    const daysBack = parseDaysBack(args.days_back, 365);
