...
async function runTool(name, args, onProgress) {
  ...
  if (name === "country_matchup_record") {
    ...
    const rows = await queryRows(
      ...
      SELECT
        ...
        m.score_sets,
        CASE
          WHEN m.winner_team_id = m.team1_id AND m.team1_country_code = ANY($1::text[]) THEN $3::text
          WHEN m.winner_team_id = m.team2_id AND m.team2_country_code = ANY($1::text[]) THEN $3::text
          WHEN m.winner_team_id = m.team1_id AND m.team1_country_code = ANY($2::text[]) THEN $4::text
          WHEN m.winner_team_id = m.team2_id AND m.team2_country_code = ANY($2::text[]) THEN $4::text
          ELSE NULL
        END AS winner_country
      FROM mart.match_mart m
      WHERE (
        (m.team1_country_code = ANY($1::text[]) AND m.team2_country_code = ANY($2::text[]))
        OR
        (m.team1_country_code = ANY($2::text[]) AND m.team2_country_code = ANY($1::text[]))
      )
      AND ($6::date IS NULL OR m.match_date >= $6::date)
      ${genderSql}
      ORDER BY m.match_date DESC NULLS LAST, m.match_id DESC
  ...
  if (name === "country_opponent_performance") {
    ...
    const genderSql = genderValues ? "AND (LOWER(COALESCE(m.tournament_gender::text, '')) = ANY($4::text[]))" : "";
    ...
    const baseSql = `
      WITH country_matches AS (
        SELECT
          CASE
            WHEN m.team1_country_code = ANY($1::text[]) THEN m.team2_country_code
            WHEN m.team2_country_code = ANY($1::text[]) THEN m.team1_country_code
            ELSE NULL
          END AS opponent_country,
          CASE
            WHEN m.winner_team_id = m.team1_id AND m.team1_country_code = ANY($1::text[]) THEN 1
            WHEN m.winner_team_id = m.team2_id AND m.team2_country_code = ANY($1::text[]) THEN 1
            ELSE 0
          END AS won_match,
          m.match_date
        FROM mart.match_mart m
        WHERE (m.team1_country_code = ANY($1::text[]) OR m.team2_country_code = ANY($1::text[]))
        AND ($2::date IS NULL OR m.match_date >= $2::date)
        ${genderSql}
      )
      SELECT
        opponent_country,
