...
const TOOL_DEFS = [
  {
    ...
    function: {
      ...
      description: "Return top Elo-ranked players for a country, optionally filtered by gender.",
      parameters: {
        type: "object",
        properties: {
...
async function runTool(name, args) {
  if (name === "top_players_by_country") {
    ...
    `;
    if (args.gender !== undefined && String(args.gender).trim() !== "") {
      params.push(String(args.gender).trim());
...
async function generateWithOpenAiTools(clean) {
  const system = {
    role: "system",
    content:
      "You are Volley Chat. Prefer tool calls for stats/facts and keep answers concise. " +
  ...
  if (toolCalls.length === 0) {
    return { status: 200, body: { role: "assistant", content: choice.content || "" } };
  }
  ...
  return { status: 200, body: { role: "assistant", content: finalText, tool: "structured_tools" } };
}
...
async function generateWithHfFallback(clean) {
  const first = await callHf(buildHfPrompt(clean));
  const toolCall = extractHfToolCall(first);
  if (!toolCall) return { status: 200, body: { role: "assistant", content: first } };
  const rows = await runTool("query_db", { sql: toolCall.sql });
...
async function generateChatReply(messages) {
  ...
  if (OPENAI_API_KEY) return generateWithOpenAiTools(clean);
  return generateWithHfFallback(clean);
}
