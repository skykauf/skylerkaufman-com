...
const TOOL_DEFS = [
  ...
  },
  {
    type: "function",
    function: {
      name: "top_players_by_entry_points",
      description:
        "Return top FIVB beach volleyball players by current entry/seeding ranking points, optionally filtered by country and gender.",
      parameters: {
        type: "object",
        properties: {
          country_code: { type: "string", description: "Optional 3-letter country code, e.g. USA, BRA, NOR." },
          gender: { type: "string", description: "Optional gender code, often 0/1 or M/W in the source." },
          limit: { type: "integer", description: "Max rows, default 10." },
        },
      },
...
/** Explorer sidebar grouping (human-readable); keys are tool function names. */
const TOOL_SECTION_BY_NAME = {
  active_players: "Player lists & activity",
  best_finishes_by_player: "Player lookup & profile",
  country_depth_report: "Country & depth",
  country_matchup_record: "Country & depth",
  country_opponent_performance: "Country & depth",
  explain_ranking: "Comparisons & rankings",
  find_player: "Player lookup & profile",
  full_player_profile: "Player lookup & profile",
  head_to_head_players: "Comparisons & rankings",
  inactive_players: "Player lists & activity",
  most_improved_players: "Player lists & activity",
  partnership_profile: "Partnerships",
  player_profile: "Player lookup & profile",
  player_recent_matches: "Player lookup & profile",
  player_status: "Player lookup & profile",
  player_teammate_history: "Comparisons & rankings",
  query_db: "SQL",
  similar_players: "Comparisons & rankings",
  top_players_by_country: "Country & depth",
  top_players_by_entry_points: "Country & depth",
  tournament_lookup: "Tournaments",
  tournament_snapshot: "Tournaments",
};

function toolExplorerSectionForName(name) {
  if (typeof name === "string" && TOOL_SECTION_BY_NAME[name]) return TOOL_SECTION_BY_NAME[name];
  return "Other";
}

/**
 * Serializable catalog of tools exposed to the model (for UI / docs).
 * Derived from TOOL_DEFS so it stays aligned with actual tool calls.
 * Sections and tool names are ordered alphabetically; parameters per tool are alphabetical.
 */
function getChatToolCatalog() {
  const tools = TOOL_DEFS.map((def) => {
    ...
    const parameters = Object.keys(props)
    ...
    return {
      name,
      section: toolExplorerSectionForName(name),
  ...
  const sectionTitles = [...bySection.keys()].sort((a, b) => a.localeCompare(b));
  const sections = sectionTitles.map((title) => ({
    title,
    tools: bySection.get(title).sort((a, b) => a.name.localeCompare(b.name)),
  }));

  return { sections, tools: tools.sort((a, b) => a.name.localeCompare(b.name)) };
}
...
async function runTool(name, args, onProgress) {
  ...
  if (name === "top_players_by_country") {
    ...
    let sql = `
      SELECT
        ...
        ROUND(COALESCE(pep.entry_seeding_points, 0)::numeric, 1) AS entry_seeding_points,
        ROUND(COALESCE(pep.technical_points_365d, 0)::numeric, 1) AS technical_points_365d,
        matches_played,
        wins,
        losses,
      ...
      LEFT JOIN staging.stg_fivb_player_entry_points pep ON pep.player_id = mart.player_elo_latest.player_id
  ...
  if (name === "top_players_by_entry_points") {
    const limit = clampLimit(args.limit, 10);
    const params = [];
    let sql = `
      SELECT
      ...
      LEFT JOIN staging.stg_fivb_player_entry_points pep ON pep.player_id = p.player_id
    ...
    params.push(limit);
    sql += ` ORDER BY COALESCE(pep.entry_seeding_points, 0) DESC, pel.elo_rating DESC NULLS LAST LIMIT $${params.length}`;
    const out = await queryRows(sql, params);
    const summary = summarizeToolOutput(out);
    emitProgress(onProgress, {
      type: "tool_done",
  ...
  if (name === "player_profile") {
    ...
    const out = await queryRows(
      ...
      country_metric_ranks AS (
        SELECT
          p.player_id,
          p.country_code,
          p.gender,
          RANK() OVER (
            PARTITION BY p.country_code, p.gender
            ORDER BY p.elo_rating DESC NULLS LAST
          ) AS country_elo_rank,
          RANK() OVER (
            PARTITION BY p.country_code, p.gender
            ORDER BY p.entry_seeding_points DESC NULLS LAST
          ) AS country_entry_points_rank
        FROM (
          SELECT
            sp.player_id,
            sp.country_code,
            sp.gender,
            pel.elo_rating,
            COALESCE(pep.entry_seeding_points, 0) AS entry_seeding_points
          FROM staging.stg_fivb_players sp
          LEFT JOIN mart.player_elo_latest pel ON pel.player_id = sp.player_id
          LEFT JOIN staging.stg_fivb_player_entry_points pep ON pep.player_id = sp.player_id
        ) p
      )
      SELECT
        c.player_id,
      ...
      LEFT JOIN staging.stg_fivb_player_entry_points pep ON pep.player_id = c.player_id
      LEFT JOIN country_metric_ranks r ON r.player_id = c.player_id
      `,
  ...
  if (name === "most_improved_players") {
    ...
    let sql = `
      ...
      SELECT
        ...
        ROUND(COALESCE(pep.entry_seeding_points, 0)::numeric, 1) AS current_entry_seeding_points,
        pa.baseline_date,
        pa.latest_date
      FROM paired pa
...
async function generateWithOpenAiTools(clean, onProgress, options = {}) {
  ...
  const system = {
    ...
    content:
      ...
      "Never promise a future action you cannot execute in the same response (for example, avoid phrasing like 'let me do that next'). " +
      "If a tool call fails, clearly state the failure and either use successful tool outputs already available or ask a concise clarification. " +
  ...
  // One retry round: if any first-pass tool failed, let the model issue follow-up
  // tool calls using the error context before producing the final user response.
  const firstPassHadToolError = toolRuns.some((r) => !r.ok);
  if (firstPassHadToolError) {
    const retryStart = Date.now();
    emitProgress(onProgress, { type: "model_start", stage: "retry_tools", provider: meta.provider, model: meta.model });
    ...
    emitProgress(onProgress, { type: "model_done", stage: "retry_tools", duration_ms: Date.now() - retryStart });

    const retryChoice = retry.choices?.[0]?.message || {};
    const retryToolCalls = Array.isArray(retryChoice.tool_calls) ? retryChoice.tool_calls : [];
...
async function runToolSmokeTests() {
  ...
  const cases = [
    ...
    { name: "top_players_by_entry_points", args: { country_code: country, limit: 5 } },
...
module.exports = { generateChatReply, runToolSmokeTests, getChatToolCatalog };
