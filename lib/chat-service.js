...
async function runTool(name, args, onProgress) {
  ...
  if (name === "country_matchup_record") {
    ...
    const rows = await queryRows(
      ...
      ${genderSql}
      ORDER BY m.match_date DESC NULLS LAST, m.match_id DESC
      LIMIT $${genderValues ? 5 : 4}
      `,
      genderValues ? [a, b, since, genderValues, limit] : [a, b, since, limit]
    );
  ...
  if (name === "country_opponent_performance") {
    ...
    const genderValues = normalizeGenderFilter(args.gender);
    const params = [country, since, limit];
    const genderSql = genderValues ? "AND (LOWER(COALESCE(m.tournament_gender::text, '')) = ANY($4::text[]))" : "";
    ...
    const rows = await queryRows(
      ...
      WITH country_matches AS (
        SELECT
          CASE
            WHEN m.team1_country_code = $1 THEN m.team2_country_code
            WHEN m.team2_country_code = $1 THEN m.team1_country_code
            ELSE NULL
          END AS opponent_country,
          CASE
            WHEN m.winner_team_id = m.team1_id AND m.team1_country_code = $1 THEN 1
            WHEN m.winner_team_id = m.team2_id AND m.team2_country_code = $1 THEN 1
            ELSE 0
          END AS won_match,
          m.match_date
        FROM mart.match_mart m
        WHERE (m.team1_country_code = $1 OR m.team2_country_code = $1)
